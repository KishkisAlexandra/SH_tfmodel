...
HEATING_MONTHS = [1,2,3,4,10,11,12]  # только эти месяцы учитываем отопление
...
# ------------------------
# Ввод реальных расходов (по желанию)
# ------------------------
...
# ------------------------
ideal_vol = calculate_volumes(area_m2, occupants, 1.0, month=month)  # идеальный расчет без поведения
...
for cat in CATEGORIES[:-1]:  # Фикс. платежи отдельно
...
# Для фиксированных платежей можно дать отдельную рекомендацию
st.write(f"- Фикс. платежи: проверьте состав услуг и тарифы для вашего дома, можно снизить расходы при оптимизации обслуживания.")
