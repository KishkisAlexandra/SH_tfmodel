...
# Рекомендации
# ------------------------
st.header("💡 Рекомендации по экономии")
...
# Функция для цвета карточки в зависимости от перерасхода
def get_color(diff):
    if diff > 0:
        return "#FFCDD2"  # красный оттенок для перерасхода
    else:
        return "#C8E6C9"  # зеленый оттенок для нормального расхода

# Создаем 4 колонки (по одной карточке на категорию)
cols = st.columns(4)
for i, cat in enumerate(["Электроэнергия","Вода","Отопление","Канализация"]):
    diff = user_real[cat] - ideal_costs[cat]
    
    # Составляем текст рекомендации
    if diff > 0:
        msg = f"На {diff:.2f} BYN выше нормы — {tips_map[cat]}"
    else:
        msg = "Расход в норме."
