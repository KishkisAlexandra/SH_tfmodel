...
def apply_neighbor_adjustment(volumes, costs, house_category):
    """Коррекция расходов для Среднего соседа"""
    coefs = HOUSE_COEFS[house_category]
    adjusted = volumes.copy()

    # Корректируем отопление и электричество
    adjusted["Электроэнергия"] *= coefs["electricity"]
    adjusted["Отопление"] *= coefs["heating"]

    # Пересчёт стоимости
    neighbor_costs = calculate_costs(adjusted, tariffs, subsidy=False)
...
# Средний сосед (с учётом возраста дома и uplift)
...
with col1:
    ...
    st.metric("Средний сосед, BYN", f"{neighbor_costs['Итого']}")
    # Процент отклонения
    diff_real = round((user_real['Итого']/ideal_costs['Итого']-1)*100 if ideal_costs['Итого']>0 else 0,1)
    diff_typical = round((user_real['Итого']/neighbor_costs['Итого']-1)*100 if neighbor_costs['Итого']>0 else 0,1)
...
with col2:
    cost_df = pd.DataFrame({
        ...
        "Средний сосед": list(neighbor_costs.values())[:-1]
    })
