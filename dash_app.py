...
if city == "Минск":
    ...
    categories = ["Электроэнергия","Вода","Отопление","Канализация"]

elif city == "Лимасол":
    emoji_map = {"Электроэнергия":"💡","Вода":"🚰","Интернет":"🌐","Телефон":"📞","IPTV":"📺","Обслуживание":"🏢"}
    tips_map = {
    ...
    categories = ["Электроэнергия","Вода","Интернет","Телефон","IPTV","Обслуживание"]

def get_color(diff):
    if diff > 0: return "#FFCDD2"  # перерасход — красный
    ...
    with cols[i]:
        st.markdown(f"""
            <div style='padding:12px; border-radius:10px; background-color:{get_color(diff)};
                        font-size:0.9em; text-align:center; min-height:130px;'>
