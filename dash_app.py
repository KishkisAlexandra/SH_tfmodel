...
# ---- Расчёт для пользовательского профиля ----
...
# ---- Расчёт типового архетипа (для сравнения) ----
...
# ---- Таблицы сравнения ----
st.subheader("Сравнение вашего потребления с типовым")
compare_df = pd.DataFrame({
    "Показатель": ["Электроэнергия (kWh)","Вода (m³)","Горячая вода (m³)","Канализация (m³)","Отопление (Gcal)","Итого (BYN)"],
    "Ваше": [user_volumes["electricity_kWh"], user_volumes["water_m3"], user_volumes["hot_water_m3"],
             user_volumes["sewage_m3"], user_volumes["heating_Gcal_month_mid"], user_costs["total_monthly"]],
    "Типовое": [typical_volumes["electricity_kWh"], typical_volumes["water_m3"], typical_volumes["hot_water_m3"],
                typical_volumes["sewage_m3"], typical_volumes["heating_Gcal_month_mid"], typical_costs["total_monthly"]]
})
st.dataframe(compare_df)

# ---- Графики ----
st.subheader("Графическое сравнение расходов")
chart_df = pd.DataFrame({
    "Ваши расходы": [user_costs["electricity_cost"], user_costs["water_cost"], user_costs["heating_cost"]],
    "Типовые расходы": [typical_costs["electricity_cost"], typical_costs["water_cost"], typical_costs["heating_cost"]]
