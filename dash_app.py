...
for cat in ["Электроэнергия", "Вода", "Отопление", "Канализация"]:
    ...
    if real > ideal:
        diff = real - ideal
        recs.append(f"{cat}: Ваш расход на {diff:.2f} BYN выше нормативного — можно оптимизировать потребление, проверив приборы и привычки.")
    elif real > neighbor:
        diff = real - neighbor
        recs.append(f"{cat}: Расход выше среднего соседа на {diff:.2f} BYN — можно немного сократить использование ресурсов.")
    else:
        recs.append(f"{cat}: Расход ниже среднего соседа — вы используете ресурсы эффективно.")
