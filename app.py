...
# Выбор города
# ------------------------
cities = ["Minsk", "Limassol"]
city = st.sidebar.selectbox("Выберите город", cities)

# Путь к YAML файлу (в корне проекта)
yaml_file = f"{city.lower()}.yaml"

if not os.path.exists(yaml_file):
    st.error(f"Файл конфигурации для города {city} не найден: {yaml_file}")
    st.stop()

with open(yaml_file, "r", encoding="utf-8") as f:
    city_config = yaml.safe_load(f)

tariffs = city_config["tariffs"]
coeffs = city_config["norms"]
house_coefs = city_config["house_coeffs"]
...
subsidy_rate = st.sidebar.slider("Доля от полного тарифа", 0.0, 1.0, 0.85, 0.05) if use_subsidy else 1.0
